...
export async function getHyperliquidTradingData() {
  ...
  try {
    ...
    // 3. Fetch Spot State (Actual Cash Balance)
    ...
    // Total Value = Spot Cash + Perp Account Value (Margin + Unrealized)
    const totalEquity = spotUsdc + parseFloat(userState.marginSummary?.accountValue || 0);
    const availableToTrade = parseFloat(userState.withdrawable || 0); // This is what the UI shows as 'Available'

    return {
      positions,
      history: (history || []).slice(0, 10),
      equity: totalEquity,
      available: availableToTrade // New field
    };
  } catch (error) {
